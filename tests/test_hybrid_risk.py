...
def test_boundary_box_trigger():
    ...
    with patch.object(eo, '_compute_flash_crash_gate', return_value=0.0):
        ...
        assert result['target_exposure'] == 0.0, "VIX > 45 should trigger hard stop (0.0 exposure)"
        assert 'Boundary Box Exceeded' in result['reason']
...
def test_vol_surface_melting():
    ...
    assert 'Vol-Surface Melting' in result['reason']
